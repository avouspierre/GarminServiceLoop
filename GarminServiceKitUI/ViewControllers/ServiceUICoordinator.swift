...
class ServiceUICoordinator: UINavigationController, ServiceOnboarding, CompletionNotifying {
    ...
    private var service: GarminService?

    init(service: GarminService? = nil, colorPalette: LoopUIColorPalette) {
    ...
    private func viewControllerForScreen(_ screen: ServiceScreen) -> UIViewController {
        ...
        case .login:
            ...
            let model = GarminDevicesAttachViewModel(service: service!)
            model.didSucceed = completeLogin
            model.didCancel = completeLogout
            let view = GarminDeviceAttachView(viewModel: model, allowCancel: true)
            let hostedView = hostingController(rootView: view)
            return hostedView
        case .status:
            let model = GarminDevicesAttachViewModel(service: service!)
            //viewModel.didLogout = completeLogout
            let view = GarminDevicesListView(viewModel: model)
            let hostedView = hostingController(rootView: view)
            return hostedView
        }
    }
